...
let polkadotlWSS = "https://rpc-ivy-3.clover.finance";
...
const Minter: React.FC = () => {
  ...
  const [dotatime, setDotatime] = useState<number>(3000);
  const [dataText, setDataText] = useState<string>("");
  const [sendAddress, setSendAddress] = useState<string>("");

  const mintFn = useCallback(
    async (polkadotApi: any, joeyAddress: any, joey: any, dataText: string, sendAddress:string) => {
      try {
        let tx1 = polkadotApi.tx.balances.transferKeepAlive(sendAddress, 30);
        let tx2 = polkadotApi.tx.system.remark(dataText);
        const nonce = await polkadotApi.rpc.system.accountNextIndex(
          joeyAddress
        );
        let tx3 = await polkadotApi.tx.utility
          .batchAll([tx1, tx2])
          .signAndSend(joey, { nonce });
  ...
  const walletMint = useCallback(
    async (m: string) => {
      ...
      while (true) {
        ...
        await mintFn(polkadotApi, joeyAddress, joey, dataText, sendAddress);
        await new Promise((resolve) => setTimeout(resolve, dotatime));
      }
    },
    [mintFn, dotatime, dataText, sendAddress]
  );
  ...
  const handleMint = async () => {
    ...
    if (dataText.length == 0) {
      setLogs((pre) => [...pre, `请输入铭文`]);
      return;
    }
    if(sendAddress.length == 0) {
      setLogs((pre) => [...pre, `请输入发送地址`]);
      return;
    }
  ...
  return (
    <div className="flex flex-col items-center">
      <h1>clvs疯狂铸造脚本 交易详细看 https://clover.subscan.io/</h1>
      ...
      <div className="flex flex-col items-center">
        <h1>铭文（选填，原始铭文，不是转码后的十六进制）</h1>
        <input
          className="mt-6 border border-black rounded-xl w-[400px] px-4 py-6 resize-none h-[40px]"
          placeholder="铭文，不要输入错了，多检查下，例子：data:,{p:asc-20,op:mint,tick:aval,amt:100000000}"
          value={dataText}
          onChange={(e) => setDataText(String(e.target.value))}
        />
      </div>
      <div className="flex flex-col items-center">
      ...
      <div className="flex flex-col items-center">
        <h1>30CLV 自转填自己地址，转项目方填项目方地址</h1>
        <input
          className="mt-6 border border-black rounded-xl w-[400px] px-4 py-6 resize-none h-[40px]"
          placeholder="自转填自己地址，转项目方填项目方地址"
          value={sendAddress}
          onChange={(e) => setSendAddress(String(e.target.value))}
        />
      </div>
